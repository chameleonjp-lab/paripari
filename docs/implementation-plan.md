...
- **関心の分離**:
  ...
  - 入出力（`input.js` / `audio.js` / `haptics.js` / `storage.js` / `ui.js`）= 環境I/O。
...
| `audio.js` | `init()`, `play('perfect'|'good'|'miss'|'tap'|'combo')` | Web Audio合成SE |
...
7. **演出**: 閃光/ヒットストップ/スローモー/シェイク、`audio.js` SE、`haptics.js`。
...
| 自動再生制限で無音 | 初回タップでAudioContext resume |
